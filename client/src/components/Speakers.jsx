...
const SpeakerCard = ({ image, name, title, company }) => (
  <div className="relative p-4 bg-white shadow-lg rounded-lg flex flex-col items-center transition-transform transform hover:scale-105 hover:shadow-xl hover:bg-gray-50">
    ...
    <h3 className="font-bold text-lg relative z-10">{name}</h3>
    <p className="text-sm text-gray-700 relative z-10">{title}</p>
    <p className="text-sm text-gray-500 relative z-10">{company}</p>
  </div>
);
...
const Speakers = () => {
  return (
    <div className="w-full space-y-8">
      ...
      <div id="speakers" className="space-y-4">
        <h2 className="font-bold text-xl">Industry Experts</h2>
        <div className="grid grid-cols-4 gap-4">
          <SpeakerCard
            image={imageLink}
            name="Jane Smith"
